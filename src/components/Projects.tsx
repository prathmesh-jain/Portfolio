    ...
    <section className='relative flex w-full pt-5 md:pb-16 pb-10 sm:scroll-mt-0 scroll-mt-60 @min-2xl:min-h-screen h-fit overflow-hidden' id='projects'>
      ...
      <div className='relative flex items-center w-full mt-16 flex-col box-border xl:px-20 md:px-10 px-5'>
        <motion.h1
          ...
          className='md:text-4xl text-3xl font-bold text-indigo-100 tracking-wide text-left w-full pl-10 font-poppins pt-10 mb-10  md:pb-5 pb-2'
        >
          Projects
        </motion.h1>
        <div className='flex w-full md:w-11/12 h-fit xl:p-5 md:p-0 sm:p-5 gap-8 md:gap-0 justify-between flex-wrap'>
          {projectsData.map((project, idx) => (
            <motion.div
              ...
              className='group rounded-3xl shadow-md relative hover:text-indigo-200 border-2 border-custom-bg text-gray-300 hover:border-indigo-700 hover:shadow-[0_0_55px_rgba(99,102,241,0.22)] cursor-pointer bg-custom-bg overflow-hidden xl:h-80 lg:h-68 md:h-56 h-72 xl:w-80 lg:w-68 md:w-56 w-72 mx-auto'
              onClick={() => openModal(project)}
            >
              <img src={project.image} alt={project.title} className='h-full w-full opacity rounded-3xl object-fill' />
        ...
        <motion.a
          ...
          target='_blank'
          className='whitespace-nowrap text-indigo-100 tracking-wide font-poppins mt-10'
        >
          View All on GitHub →
        </motion.a>
